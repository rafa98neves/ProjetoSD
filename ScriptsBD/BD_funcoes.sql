...
IF '@tipo' = 'musica'
	BEGIN
		...
		if('@info5' < 0) --Ou seja se não pertencer a nenhuma banda.
		BEGIN
		INSERT INTO dbo.Music
		(
			Musicinfo,
		...
		(
			...
			(SELECT Findartist.ArtistID
				FROM dbo.Artist Findartist
				WHERE Findartist.Name = '@info'),
			'@aux', --Boolean: 0 se o artista em questão não for o músico da mesma, 1 caso contrário
			'@aux2' --Boolean: 1 se o artista for Compositor da mesma, 1 caso contrário
		)
		END
	END	
